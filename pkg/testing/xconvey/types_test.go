...
func TestAssertionsAreAvailableFromConveyPackage(t *testing.T) {
	...
	Convey("Error assertions should be accessible", t, func() {
		inner := errors.New("I am inner error")
		So(inner, ShouldBeError)

		outer := fmt.Errorf("I am outer error %w", inner)
		So(outer, ShouldWrap, inner)
	})

	SkipConvey("SkipConvey example", t, func() {
		Convey("Convey In SkipConvey will not execute", func() {
			So(1, ShouldEqual, 0)
		})

		FocusConvey("FocusConvey In SkipConvey will execute", func() {
			So(1, ShouldEqual, 0)
		})
	})

	FocusConvey("FocusConvey example", t, func() {
		Convey("Convey In FocusConvey will not execute", func() {
			So(1, ShouldEqual, 0)
		})

		FocusConvey("FocusConvey In FocusConvey will execute", func() {
			So(1, ShouldEqual, 1)
		})
	})
}
